...
void main() {
  ...
  GetCreateTaskUseCase useCase;

  setUp(() {
    taskRepository = MockTaskRepository();
    useCase = GetCreateTaskUseCase(
  ...
  test("GetCreateTaskUseCase should extend the abstract class usecase",
          () {
        // assert
        expect(useCase, isA<UseCase>());
      });

  test("GetCreateTaskUseCase should should get data from a repository",
          () async {
        // arrange
        ...
        // assert
        verify(taskRepository.createTask(tTaskEntity));
      });

  test("GetCreateTaskUseCase if success is returned, the task list should be returned",
          () async {
        // arrange
        when(taskRepository.createTask(any))
            .thenAnswer((_) async => Right(tTaskEntity));
  ...
  test("GetCreateTaskUseCase if failure is returned, Failure should be returned",
          () async {
        // arrange
        when(taskRepository.createTask(any))
